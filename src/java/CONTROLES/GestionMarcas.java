    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (request.getParameter("boton").equals("Guardar")) {
                ObjDtoMar.setIdMarcas(Integer.parseInt(request.getParameter("idMarcas")));
                ObjDtoMar.setDescripcionMarcaVehiculo(request.getParameter("descripcionMarcaVehiculo"));

                Estado = ObjDaoMar.IngresarMarca(ObjDtoMar);

            } else if (request.getParameter("boton").equals("consultar")) {

            } else if (request.getParameter("boton").equals("Eliminar")) {
                int Pos = Integer.parseInt(request.getParameter("pos"));
                Estado = ObjDaoMar.EliminarMarca(Pos);

            } else if (request.getParameter("boton").equals("Actualizar")) {
            }
            response.sendRedirect("/ProyectoSimva/spanish/asignacion/formMarcas.jsp?Estado=" + Estado);
        }
    }
